...
Processa videos do YouTube gerando datasets de audio para TTS/STT
"""
...
from m03_legendas import processar_legendas
...
def configurar_logger(id_video: str) -> logging.Logger:
    """
    Configura logger para o video especifico
    ...
    # Arquivo de log
    arquivo_log = pasta_logs / f"{id_video}.log"
    
    # Configurar logger
    logger = logging.getLogger(f"pipeline_{id_video}")
...
def listar_ids_disponiveis() -> List[str]:
    """
    Lista IDs de videos disponiveis em ./arquivos/audios/
    ...
    ids = []
    for item in pasta_audios.iterdir():
        if item.is_dir():
...
def verificar_processado(id_video: str) -> bool:
    """
    Verifica se video ja foi processado checando historico
    ...
    """
    arquivo_historico = PROJECT_ROOT / "dataset" / "historico_dataset" / f"{id_video}.json"
...
def calcular_duracao_audios_aprovados(ids_processados: List[str]) -> float:
    ...
    for id_video in ids_processados:
        historico_path = PROJECT_ROOT / "dataset" / "historico_dataset" / f"{id_video}.json"
...
def salvar_metadados_csv(
    ...
    linha = {
        ...
        # M03
        'm03_processar_legendas_segundos': round(tempos_modulos.get('m03', 0) or 0, 2) if tempos_modulos.get('m03') is not None else 'null',
        'm03_processar_legendas_percentual': percentuais.get('m03') if percentuais.get('m03') is not None else 'null',
...
def executar_pipeline(id_video: str, logger: logging.Logger, tempos_modulos: Dict[str, Optional[float]]) -> bool:
    """
    Executa pipeline completa para um video
    ...
    """
    logger.info("="*80)
    logger.info(f"INICIANDO PIPELINE PARA VIDEO: {id_video}")
    logger.info("="*80)
    ...
    try:
        ...
        inicio = time.time()
        criar_diretorios(id_video)
        tempos_modulos['m02'] = time.time() - inicio
        logger.info(f"[M02] Concluido ({tempos_modulos['m02']:.2f}s)")
        
        # ======================================================================
        # M03/M04 - SEGMENTACAO (CONDICIONAL)
        # ======================================================================
        modo_segmentacao = MASTER.get('segmentacao', '')
        
        if modo_segmentacao == 'legenda':
            logger.info("[M03] Processando legendas...")
            inicio = time.time()
            sucesso = processar_legendas(id_video)
            tempos_modulos['m03'] = time.time() - inicio
            if not sucesso:
                logger.error("[M03] FALHOU - Abortando pipeline")
                return False
            logger.info(f"[M03] Concluido ({tempos_modulos['m03']:.2f}s)")
            
            logger.info("[M04-LEG] Executando segmentacao por legendas...")
            inicio = time.time()
            executar_segmentacao_leg(id_video)
            ...
            inicio = time.time()
            executar_segmentacao_vad(id_video)
        ...
        if MASTER.get('mos_filter', False):
            ...
            inicio = time.time()
            sucesso = processar_mos(id_video)
        ...
        inicio = time.time()
        processar_normalizacao(id_video)
        ...
        inicio = time.time()
        processar_normalizador_audio(id_video)
        ...
        # ======================================================================
        logger.info("="*80)
        logger.info(f"PIPELINE CONCLUIDA COM SUCESSO PARA: {id_video}")
        logger.info("="*80)
...
def main():
    ...
    # Executar downloader (se ativo) - UMA VEZ antes do loop
    if MASTER.get('downloader', False):
        print("[M00] Downloader ativado - executando...")
        # TODO: Implementar m00_downloader quando disponivel
        print("[M00] Downloader ainda nao implementado - pulando")
    
    # Listar IDs disponiveis
    ids_disponiveis = listar_ids_disponiveis()
    
    if not ids_disponiveis:
        print("\nNENHUM VIDEO ENCONTRADO em ./arquivos/audios/")
        print("Adicione pastas com IDs de videos para processar")
        return
    
    print(f"\nVideos encontrados: {len(ids_disponiveis)}")
    for id_video in ids_disponiveis:
        print(f"  - {id_video}")
    print()
    ...
    tempos_modulos_acumulados = {
        'm02': 0.0, 'm03': None, 'm04_leg': None, 'm04_vad': None,
        'm05': 0.0, 'm06': None, 'm07': None, 'm08': None, 'm09': None,
        'm10': 0.0, 'm11': 0.0, 'm12': None, 'm13': 0.0, 'm14': 0.0, 'm15': None
    }
    
    for idx, id_video in enumerate(ids_disponiveis, 1):
        print(f"\n[{idx}/{len(ids_disponiveis)}] Processando: {id_video}")
        print("-"*80)
        
        # Verificar se ja foi processado ANTES
        if verificar_processado(id_video):
            print(f"Video {id_video} ja processado (encontrado em historico)")
            
            # Executar cleanup se configurado
            modo_cleanup = MASTER.get('cleanup', 'none')
            if modo_cleanup in ['all', 'input']:
                print(f"Executando cleanup (modo: {modo_cleanup})...")
                logger_temp = configurar_logger(id_video)
                executar_cleanup(id_video)
            
            pulados_nesta_execucao += 1
            continue
        
        # Video NAO estava processado - contar NESTA EXECUCAO
        total_nesta_execucao += 1
        
        # Configurar logger
        logger = configurar_logger(id_video)
        
        # Dicionario para tempos deste video
        tempos_modulos_video = {}
        
        # Executar pipeline
        sucesso = executar_pipeline(id_video, logger, tempos_modulos_video)
        
        if sucesso:
            processados_nesta_execucao += 1
            ids_processados_com_sucesso.append(id_video)
            
            # Acumular tempos APENAS dos processados com sucesso
            for modulo, tempo in tempos_modulos_video.items():
            ...
            print(f"\nERRO ao processar {id_video} - verifique o log")
