...
def processar_normalizacao(audio_id: str):
    ...
    PASTA_OUTPUT_NORMALIZADO = PROJECT_ROOT / "arquivos" / "temp" / audio_id / "08-normalizador_texto"
    PASTA_OUTPUT_JSON_DINAMICO = PASTA_JSON_DINAMICO

    ARQUIVO_FILTRO = PASTA_JSON_DINAMICO / f"{audio_id}.json"
    ...
    if tem_arquivo_filtro:
        ...
        if dados_filtro:
            ...
            # Arquivo existe mas sem segmentos aprovados — respeitar o filtro
            segmentos_elegiveis = []
            logger.info("  Arquivo de filtro sem segmentos elegiveis — nada a normalizar")
    else:
    ...
    arquivo_acomp_output = PASTA_OUTPUT_NORMALIZADO / f"{audio_id}_segments_acompanhamento.json"
    ...
    if tem_arquivo_filtro and dados_elegiveis_normalizados:
        arquivo_norm_output = PASTA_OUTPUT_NORMALIZADO / f"{audio_id}_normalizado.json"
    ...
    arquivo_acomp_destino = PASTA_OUTPUT_JSON_DINAMICO / f"{audio_id}_segments_acompanhamento.json"
    ...
    if tem_arquivo_filtro and dados_elegiveis_normalizados:
        arquivo_norm_source = PASTA_OUTPUT_NORMALIZADO / f"{audio_id}_normalizado.json"
        arquivo_norm_destino = PASTA_OUTPUT_JSON_DINAMICO / f"{audio_id}.json"
